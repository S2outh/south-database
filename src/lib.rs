...
pub async fn run(config: config::DBConfig) -> Result<(), DBError> {
    ...
    for topic in config.topics {
        let subscription = nats_client.subscribe(topic).await.map_err(|e| DBError::SubscribeNATS(e))?;
        let database_client = SenderBuilder::new(Protocol::Http, &config.db_address, 9000).build().map_err(|e| DBError::ConnectDB(e))?;
...
async fn subscriber_loop(mut subscription: Subscriber, mut database_client: Sender) -> Result<(), DBError> {
    while let Some(msg) = subscription.next().await {
        ...
        match telestion_parser::to_buffer(msg.subject.as_str(), msg.payload.as_ref()) {
            Ok(mut buffer) => database_client.flush(&mut buffer).map_err(|e| DBError::SendDB(e))?,
